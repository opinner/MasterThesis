...
color_scheme = [green_iterator,blue_iterator,red_iterator]
...
for index in range(3):
    ...
    print("\n\n"+cruisename)
    for a,b in zip(bin_rolling_mean_Shih,bin_rolling_mean_Shih_wob):
        if a!=b and not np.isnan(a):
            print(a,b)
            
    #axis.plot(bin_longitude,bin_raw_Osborn,"x")
    #axis.plot(bin_longitude,bin_rolling_mean_Osborn, "-", c = color, label = cruisename+" Osborn with BBL")
    axis.plot(bin_longitude,bin_rolling_mean_Osborn, "-", c = next(color), label = cruisename+" Osborn with BBL", lw = 2.5)
    ...
    axis.plot(bin_longitude_wob,bin_rolling_mean_Osborn_wob, ":", c = next(color), label = cruisename+" Osborn", lw = 2.5)
...
axis.legend(handles=[emb169_label,emb177_label,emb217_label,BBL_label,wo_BBL_label,bathy_label],loc = "lower left")
...
output.subplots_adjust(top=0.929,bottom=0.131,left=0.133,right=0.893,hspace=0.058,wspace=0.185)
output.savefig("/home/ole/Thesis/Analysis/mss/pictures/statistics/BBL_comparison", dpi = 600)
