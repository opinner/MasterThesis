...
for index in range(3):
    ...
    bin_longitude,bin_distance,bin_bathymetry,bin_raw_Osborn,bin_rolling_mean_Osborn,bin_raw_Shih,bin_rolling_mean_Shih = np.loadtxt("./data/"+cruisename+"_iso_flux_results.txt", unpack=True)
    bin_longitude_wob,bin_distance_wob,bin_bathymetry_wob, bin_raw_Osborn_wob,bin_rolling_mean_Osborn_wob,bin_raw_Shih_wob,bin_rolling_mean_Shih_wob = np.loadtxt("./data/"+cruisename+"_iso_flux_results_woBBL.txt", unpack=True)
...
axis.set_ylabel(r"$\langle$Oxygen flux$\rangle$ [mmol/(m$^2$d)]")
