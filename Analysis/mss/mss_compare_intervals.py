...
for FOLDERNAME in LIST_OF_MSS_FOLDERS:
    ...
    #TODO define figure here
    f_dissip,dissip_axarr = plt.subplots(nrows = 3, ncols = 1, sharey = True, sharex = True) 
               
    f_flux,flux_axarr = plt.subplots(nrows = 3, ncols = 1, sharey = True, sharex = True) 
    
    figure_index = 0
        
    for upper_bound_halocline_as_density,lower_bound_halocline_as_density in zip(upper_bounds,lower_bounds):
        ...
        bathymetrie_axes = flux_axarr[figure_index].twinx()
        
        #define flux_axarr as the foreground
        flux_axarr[figure_index].set_zorder(10)
        flux_axarr[figure_index].patch.set_visible(False)
            
        
        #flux_axarr[figure_index].plot(longitude_list,mean_flux,"x", c = "tab:green", alpha = 0.4, label = "mean flux")                
        flux_axarr[figure_index].plot(longitude_list,rolling_mean_flux, lw = 3, zorder = 3, c = "k", label = "rolling mean flux")#"tab:blue")
        #flux_axarr.plot(longitude_list,rolling_median_flux,"--",zorder = 3, c = "k")#"tab:blue")
        

        flux_axarr[figure_index].fill_between(longitude_list,rolling_upper_percentile_flux,rolling_lower_percentile_flux, color = "tab:blue", zorder = 2, alpha = 0.7, label = str(flux_percentile)+"% percentile")
        flux_axarr[figure_index].fill_between(longitude_list,rolling_upper_percentile_flux,rolling_second_upper_percentile_flux, color = "tab:blue", zorder = 2, alpha = 0.4, label = str(second_flux_percentile)+"% percentile")
        flux_axarr[figure_index].fill_between(longitude_list,rolling_lower_percentile_flux,rolling_second_lower_percentile_flux, color = "tab:blue", zorder = 2, alpha = 0.4)
        ...
        flux_axarr[figure_index].legend(handles=[bathymetrie_label]) #loc=8

        flux_axarr[figure_index].set_ylim((-20,1))
        if cruisename == "emb177":
            flux_axarr[figure_index].set_ylim((-50,1))    
        flux_axarr[figure_index].set_title(str(upper_bound_halocline_as_density)+r"< $\sigma$ < "+str(lower_bound_halocline_as_density)+r" kg/m$^3$", fontweight="bold")       
        flux_axarr[figure_index].legend()
        ...
        dissip_axarr[figure_index].set_zorder(10)
        dissip_axarr[figure_index].patch.set_visible(False)
        
        bathymetrie_axes2 = dissip_axarr[figure_index].twinx()
        ...
        #dissip_axarr[figure_index].plot(longitude_list,log_mean_dissipation,"x", c = "tab:green", alpha = 0.4, label = "mean dissipation")        
        #dissip_axarr[figure_index].plot(longitude_list,rolling_log_mean_dissipation, "k", label ="rolling logarithmic mean")
        dissip_axarr[figure_index].plot(longitude_list,rolling_arith_mean_dissipation, "k--", label ="rolling arithmic mean")

        dissip_axarr[figure_index].fill_between(longitude_list,rolling_upper_percentile_dissip,rolling_lower_percentile_dissip, color = "tab:blue", alpha = 0.7, label=str(dissip_percentile)+"% percentile") 
        dissip_axarr[figure_index].fill_between(longitude_list,rolling_upper_percentile_dissip,rolling_second_upper_percentile_dissip, color = "tab:blue", alpha = 0.4, label=str(second_dissip_percentile)+"% percentile")
        dissip_axarr[figure_index].fill_between(longitude_list,rolling_lower_percentile_dissip,rolling_second_lower_percentile_dissip, color = "tab:blue", alpha = 0.4) 
        dissip_axarr[figure_index].legend()
        dissip_axarr[figure_index].set_title(str(upper_bound_halocline_as_density)+r"< $\sigma$ < "+str(lower_bound_halocline_as_density)+r" kg/m$^3$", fontweight="bold")
        figure_index += 1
    
    
    f_flux.set_size_inches(18,10.5)
    f_flux.tight_layout() 
    f_flux.subplots_adjust(top=0.915, left = 0.055)

    flux_axarr[2].set_xlabel(r"longitude [$\degree$]")    
    flux_axarr[1].set_ylabel(r"oxygen flux [mmol/(m$^2$*d]")

    titlestring = cruisename+": rolling mean BB oxygen flux (over "+str(rolling_window_size)+" points)"
    f_flux.suptitle(titlestring,fontweight="bold") 
    f_flux.savefig("/home/ole/Thesis/Analysis/mss/pictures/statistics/"+cruisename+"_running_mean_flux_interval_comparison")           
                        
    dissip_axarr[1].set_ylabel(r"log10($\epsilon$) $[m^2 s^{-3}]$")   
    dissip_axarr[2].set_xlabel("longitude [degree]")       
    """      
    ...
    f_dissip.subplots_adjust(top=0.915)

    titlestring = cruisename+": rolling mean dissipation (over "+str(rolling_window_size)+" points)"
    f_dissip.suptitle(titlestring,fontweight="bold")    
    f_dissip.savefig("/home/ole/Thesis/Analysis/mss/pictures/statistics/"+cruisename+"_running_mean_dissipation_interval_comparison")
