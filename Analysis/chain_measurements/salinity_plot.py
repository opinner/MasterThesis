...
DATAFILENAMES = []
#go through all files of specified folder and select only files ending with "salinity.npz"
for p in path.iterdir():
    FILENAME = str(p.parts[-1])
    splitted_filename = FILENAME.split("_")
    ending = splitted_filename[-1]
    
    if (ending == "salinity.npz"): 
...
for DATAFILENAME in DATAFILENAMES:
    ...
    print(np.shape(salinity),np.shape(label_list),np.shape(utc))
   
    """
    print("original:")
    for depth_index in range(np.shape(salinity)[0]):
        print(depth_index,depth[depth_index],np.nanmean(salinity[depth_index,:]))
    """
    ...
    for index in sorting_key[:,1]:
        sorted_salinity.append(salinity[int(index),:])
    sorted_salinity = np.asarray(sorted_salinity)
    
    print("sorted")
    for depth_index in range(np.shape(sorted_salinity)[0]):
        print(depth_index,sorted_depth[depth_index],np.nanmean(sorted_salinity[depth_index,:]))
    ...
    img1_1 = axarr1[y_position,x_position].pcolormesh(utc,plot_edges_depth,sorted_salinity, cmap = plt.cm.RdYlBu_r)
...
f1.suptitle("salinity from seabird comparison")
...
plot1_name = "./pictures/"+"salinity_comparison" 
...
plot1_name = "./pictures/"+"salinity_comparison_same_scale" 
f1.savefig(plot1_name)
